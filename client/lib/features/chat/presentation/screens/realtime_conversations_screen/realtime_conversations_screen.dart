...
class _RealtimeConversationsScreenState
              ...
              children: [
                ...
                StreamBuilder(
                  ...
                  builder: (context, snapshotContacts) {
                    ...
                    final contacts = snapshotContacts.data!.where((element) =>
  ...
  List<DetailedConversation> filteredConversations(
      List<DetailedConversation> conversations) {
    String _unformat(String? text) =>
        (text ?? '').toLowerCase().replaceAll(' ', '');

    return conversations
        .where((element) =>
            _unformat(element.messages.lastOrNull?.text)
                    .contains(_unformat(searchController.text)) ==
                true ||
            _unformat(element.title)
                .contains(_unformat(searchController.text)) ||
            element.users.length <= 1 ||
            element.users.any((user) => _unformat(user.fullName)
                .contains(_unformat(searchController.text))))
        .toList();
  }
}
