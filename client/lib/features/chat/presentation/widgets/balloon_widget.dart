...
typedef NewConstraints = BoxConstraints? Function(
...
class BalloonWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final curve = showCurve
        ? Padding(
            padding: isLeftSide
                ...
                : const EdgeInsets.only(left: kWidth),
            child: Transform(
    ...
    return Align(
      alignment:
          rightSideChild != null ? Alignment.bottomRight : Alignment.bottomLeft,
      child: LayoutBuilder(
        builder: (context, constraints) {
          return Padding(
              ...
              child: Row(
                ...
                children: [
                  if (leftSideChild != null) leftSideChild,
                  Container(
                      clipBehavior: Clip.none,
                      padding: const EdgeInsets.only(
                          left: 11, right: 11, top: 6, bottom: 2),
                      decoration: BoxDecoration(
                        color: rightSideChild != null
                            ? Colors.indigo[700]
                            : Colors.white,
                        boxShadow: [
                          BoxShadow(
                            ...
                            blurRadius: 2,
                            offset: Offset(
                                5 * (rightSideChild != null ? -1 : 1),
                                5), // changes position of shadow
                          ),
                        ],
                        borderRadius: BorderRadius.only(
                          topLeft: rightSideChild != null || !showCurve
                              ? _curveRadius
                              : Radius.zero,
                          bottomRight: _curveRadius,
                          bottomLeft: _curveRadius,
                          topRight: rightSideChild != null
                              ? Radius.zero
                              : _curveRadius,
                        ),
                      ),
                      child: Column(
                        ...
                        children: [
                          if (centerChild != null)
                            Container(
                              constraints: centerChildConstraints == null
                                  ? constraints
                                  : centerChildConstraints!(constraints),
                              child: centerChild!,
                            ),
                        ],
                      )),
...
class _SideWidgetClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    path.quadraticBezierTo(
        size.width * 0.75, size.height / 6, size.width, size.height);
