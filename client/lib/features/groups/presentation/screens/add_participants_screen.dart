...
import 'package:client/core/presentation/widgets/person_icon.dart';
import 'package:client/core/presentation/widgets/snackbar.dart';
import 'package:client/features/groups/presentation/controllers/add_participants_controller.dart';
...
class AddGroupParticipantsScreenArgs {
  // group conversation id that new particpants will be added
...
class AddGroupParticipantsScreen extends StatefulWidget {
  ...
  @override
  State<AddGroupParticipantsScreen> createState() =>
...
class _AddGroupParticipantsScreenState
  ...
  // we consider this class initialized when the 'args' value is set
  bool initialized = false;

  // controller handles all the state changes
  // ui only renders according to the states
  ...
  void didChangeDependencies() {
    ...
    args = ModalRoute.of(context)!.settings.arguments
        as AddGroupParticipantsScreenArgs;
    initialized = true;
    controller =
        AddParticipantsController(conversationId: args!.conversationId);
  ...
  Widget build(BuildContext context) {
    ...
    return MyScaffold(
      appBar: MyAppBarWidget(
        context: context,
        child: const Text('Adding Participants',
            style: TextStyle(
                color: Colors.white,
                fontSize: 17,
      ...
      padding: const EdgeInsets.only(
          top: 1, bottom: kMargin, right: kMargin, left: kMargin),
      body: ValueListenableBuilder(
          valueListenable: controller.notifySelectedUsers,
          builder: (context, users, _) {
            // number of users that is selected
            final selectCount =
                users.entries.where((entry) => entry.value.$2).length;
                    ...
                    children: [
                      ...
                      ...filteredUsers.mapIndexed((i, e) {
                        return Padding(
                          ...
                          child: InkWell(
                            onTap: () {
                              controller.selectUserChanged(
                                  e.value.$1.uid, !e.value.$2);
                            },
                            child: Ink(
                              child: Column(
                                children: [
                                                  ...
                                                  const PersonIcon(
                                                      ...
                                                      iconInternalPadding:
                                                          EdgeInsets.all(
                                                              paddingSize2)),
                                                const SizedBox(
                                                  width: spaceAfterIcon,
                                                ),
                                              ],
                                            ),
                                            Text(e.value.$1.fullName,
                                                style: TextStyle(
                                                    color: Colors.white,
                                                    fontSize: 17,
                    ...
                    children: [
                      if (selectCount > 0) ...[
                        ButtonWidget(
                          ...
                          onPressed: () {
                            controller.addParticipants().then((_) {
                              showSnackbar(
                                  message: selectCount == 1
                                      ? "1 participants added to ${controller.conversation!.group!.title}"
                                      : "$selectCount participants added to ${controller.conversation!.group!.title}",
                                  icon: Icons.check);
                            });
                      ...
                      ButtonWidget(
                        text: controller.outsideGroupUsers?.isEmpty == true
                            ? "CLOSE"
                            : "CANCEL",
                        onPressed: () {
                          Navigator.of(context).pop();
  ...
  bool checkIfMatchesFilter(MapEntry<String, (UserPublic, bool)> element) {
    ...
    return unformatted(element.value.$1.fullName)
        .contains(unformatted(searchController.text));
  }
}
